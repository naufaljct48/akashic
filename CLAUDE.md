...
Vector retrieval is **live**. `comic_embeddings` holds a 1024-dim `@cf/baai/bge-m3` vector for every catalog row, generated by `bun run embed:catalog` ([embed-catalog.ts](scripts/embed-catalog.ts)) via Cloudflare Workers AI. The query side runs inside the Edge Function, which is the only caller that can: `match_comics_hybrid` is `service_role`-only by migration `20260903`.

Three things about it are load-bearing and easy to undo by accident:

- **`SET hnsw.ef_search = 400` on the RPC.** pgvector's default is 40, and an HNSW index never returns more rows than that — silently. With the default, a query whose true rank-2 answer scored 0.5814 got a pool that never contained it at all. Recall@20 measured 38% before this and 86% after.
- **The ranking blends popularity**, `log10(1+popularity)/6` at weight 0.15, over a 200-row shortlist. Pure cosine across ~18,000 rows loses to lexical accidents: "raksasa yang memakan manusia di balik tembok" ranked Attack on Titan below *Eat-Man* and *EAT*.
- **Indonesian queries are translated to English before embedding.** bge-m3 is multilingual, but at catalog scale an Indonesian query sits far enough off the English document cluster to scramble ranking. Translating lifted recall@5 from 67% to 86%.

Never claim a retrieval change is an improvement without running `bun run eval:recall` ([eval-recall.ts](scripts/eval-recall.ts)) — 21 description-shaped queries pinned to AniList `source_id`. Add `--translate` to measure the production path.
